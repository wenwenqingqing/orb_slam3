...
namespace ORB_SLAM3
{
    ...
    Viewer::Viewer(System *pSystem, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Tracking *pTracking, const string &strSettingPath) : both(false), mpSystem(pSystem), mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpTracker(pTracking),
    ...
    void Viewer::Draw()
    {
        ...
        if (mbStopTrack)
        {
            std::cout << "if mbStop trak == true" << std::endl;
            menuStepByStep = true;
            mbStopTrack = false;
        }
        ...
        {
            std::cout << "mpTracker->SetStepByStep(true);" << std::endl;
