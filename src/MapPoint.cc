...
namespace ORB_SLAM3
{
    ...
    void MapPoint::Replace(MapPoint *pMP)
    {
        ...
        int nvisible, nfound;
        std::cout << "map<KeyFrame*,tuple<int,int>> obs;" << mObservations.size() << std::endl;
        map<KeyFrame *, tuple<int, int>> obs;
        {
            unique_lock<mutex> lock1(mMutexFeatures);
            unique_lock<mutex> lock2(mMutexPos);
        ...
        for (map<KeyFrame *, tuple<int, int>>::iterator mit = obs.begin(), mend = obs.end(); mit != mend; mit++)
        {
            ...
            int leftIndex = get<0>(indexes), rightIndex = get<1>(indexes);
            std::cout << "if(!pMP->IsInKeyFrame(pKF))" << std::endl;
            if (!pMP->IsInKeyFrame(pKF))
            {
                std::cout << "if(!pMP->IsInKeyFrame(pKF)) 1" << std::endl;
                if (leftIndex != -1)
                {
            ...
            {
                std::cout << "if(!pMP->IsInKeyFrame(pKF)) 2" << std::endl;
                if (leftIndex != -1)
                {
                    pKF->EraseMapPointMatch(leftIndex);
                }
        ...
        std::cout << "pMP->IncreaseFound(nfound)" << std::endl;
        pMP->IncreaseFound(nfound);
        std::cout << "pMP->IncreaseVisible(nvisible);" << std::endl;
        pMP->IncreaseVisible(nvisible);
        std::cout << "ComputeDistinctiveDescriptors();" << std::endl;
        pMP->ComputeDistinctiveDescriptors();
        std::cout << "EraseMapPoint(this);" << std::endl;
        mpMap->EraseMapPoint(this);
        std::cout << "pMP->IncreaseFound(nfound) end" << std::endl;
    }
    ...
    void MapPoint::ComputeDistinctiveDescriptors()
    {
        ...
        for (map<KeyFrame *, tuple<int, int>>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
        {
            ...
            if (!pKF->isBad())
            {
                std::cout << "MP: one step !pKF->isBad" << std::endl;
                tuple<int, int> indexes = mit->second;
                int leftIndex = get<0>(indexes), rightIndex = get<1>(indexes);

                if (leftIndex != -1)
                {
                    std::cout << "MP: one step !pKF->isBad" << leftIndex << std::endl;
                    vDescriptors.push_back(pKF->mDescriptors.row(leftIndex));
                }
                if (rightIndex != -1)
                {
                    std::cout << "MP: one step !pKF->isBad" << rightIndex << std::endl;
        ...
        if (vDescriptors.empty())
            return;
        std::cout << "MP: one step not empty" << std::endl;
        // Compute distances between them
        const size_t N = vDescriptors.size();
        std::cout << "MP: one step" << N << std::endl;
        float Distances[N][N];
        for (size_t i = 0; i < N; i++)
        {
            ...
            for (size_t j = i + 1; j < N; j++)
            {
                ...
                std::cout << "MP: one step" << i << " ," << j << " ,distij: " << distij << std::endl;
        ...
        for (size_t i = 0; i < N; i++)
        {
            ...
            std::cout << "MP: one step median" << median << " N: " << N << std::endl;
        ...
        std::cout << "MP: one step end" << std::endl;
    }
