    ...
    void LocalMapping::Run()
    {
        ...
        while (1)
        {
            ...
            else if (Stop() && !mbBadImu)
            {
                ...
                std::cout << "LM: stop check finish" << std::endl;
                if (CheckFinish())
                    break;
            }
            ...
            // std::cout << "LM: local mapping set accept key frame" << std::endl;
            // Tracking will see that Local Mapping is busy
            SetAcceptKeyFrames(true);
            // std::cout << "LM: local mapping check finish" << std::endl;
            if (CheckFinish())
                break;
            // std::cout << "LM: local mapping loop once" << std::endl;
    ...
    void LocalMapping::CreateNewMapPoints()
    {
        ...
        for (size_t i = 0; i < vpNeighKFs.size(); i++)
        {
            ...
            for (int ikp = 0; ikp < nmatches; ikp++)
            {
                ...
                cv::Mat x3D;
                if (cosParallaxRays < cosParallaxStereo && cosParallaxRays > 0 && (bStereo1 || bStereo2 || (cosParallaxRays < 0.9998 && mbInertial) || (cosParallaxRays < 0.9998 && !mbInertial)))
                {
    ...
    void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
    {
        ...
        if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
        {
            ...
            for (vector<KeyFrame *>::iterator itKF = vpKF.begin(); itKF != vpKF.end(); itKF++)
            {
                ...
                cv::Mat _vel = ((*itKF)->GetImuPosition() - (*itKF)->mPrevKF->GetImuPosition()) / (*itKF)->mpImuPreintegrated->dT; //unscale velocity
