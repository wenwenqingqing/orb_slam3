...
namespace gtsam_wrapper
{
    ...
    * variables.  To derive from this class, implement evaluateError(). */
    ...
    public:
        ...
        NavStateGS retract(const Vector9& v,
            OptionalJacobian<9, 9> H1 = boost::none, OptionalJacobian<9, 9> H2 =
            boost::none) const;
    };

    class GTSAM_EXPORT PreintegratedImuMeasurementsGS: public gtsam::PreintegratedImuMeasurements
    {
    public:
        NavStateGS predict(const NavStateGS& state_i,
...
namespace gtsam_optimizer
{
        ...
        gtsam::Rot3, double, imuBias::ConstantBias>
    {
        ...
        typedef gtsam_wrapper::NoiseModelFactor7<gtsam::Pose3, gtsam::Vector3, gtsam::Pose3, gtsam::Vector3, 
            gtsam::Rot3, double, imuBias::ConstantBias> Base;
    ...
    public:
        InertialEgdeGS(gtsam::Key posei_key, gtsam::Key veli_key, gtsam::Key posej_key, gtsam::Key velj_key, 
            ...
            : Base(noiseModel::Gaussian::Covariance(p.PreintMeasCov()), posei_key, veli_key, posej_key, velj_key, rwg_key, scale_key, b_key)
            , _PIM_(p)
            , dt_(p.deltaTij())
        {
        }
    ...
    class GTSAM_EXPORT InertialEgdeGS2 : public gtsam_wrapper::NoiseModelFactor7<gtsam::Pose3, gtsam::Vector3, gtsam::Pose3, gtsam::Vector3, 
        gtsam::Rot3, double, imuBias::ConstantBias>
    {
        typedef InertialEgdeGS2 This;
        typedef gtsam_wrapper::NoiseModelFactor7<gtsam::Pose3, gtsam::Vector3, gtsam::Pose3, gtsam::Vector3, 
            gtsam::Rot3, double, imuBias::ConstantBias> Base;

        InertialEgdeGS2(){}
    public:
        InertialEgdeGS2(gtsam::Key posei_key, gtsam::Key veli_key, gtsam::Key posej_key, gtsam::Key velj_key, 
            gtsam::Key rwg_key, gtsam::Key scale_key, gtsam::Key b_key, const gtsam::PreintegratedImuMeasurements &p)
            : Base(noiseModel::Gaussian::Covariance(p.preintMeasCov()), posei_key, veli_key, posej_key, velj_key, rwg_key, scale_key, b_key)
            , _PIM_(p)
            , dt_(p.deltaTij())
            , gI_(p.params()->n_gravity)
        {
        }
